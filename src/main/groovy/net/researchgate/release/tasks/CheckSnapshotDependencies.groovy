...
class CheckSnapshotDependencies extends BaseReleaseTask {
    ...
    void checkSnapshotDependencies() {
        def matcher = { Dependency d -> d.version?.contains('SNAPSHOT') && !extension.ignoredSnapshotDependencies.get().contains("${d.group ?: ''}:${d.name}".toString()) }
        ...
        if (message) {
            ...
            warnOrThrow(extension.failOnSnapshotDependencies.get(), message)
        }
    }
